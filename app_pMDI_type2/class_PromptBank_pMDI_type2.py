class PromptBank:
    def __init__(self):
        ...
        self.check_action_step_pMDI_type2 = {
            'sit_stand': {
        ...
        for q_key, action_key in q_mapping.items():
            if q_key in q_answers_accumulated:
                for answer_data in q_answers_accumulated[q_key]:
                    ...
                    self.check_action_step_pMDI_type2[action_key]['time'].append(time_val)
                    self.check_action_step_pMDI_type2[action_key]['score'].append(score)
                    
                    # confidence score 저장
                    if confidence is not None:
                        self.check_action_step_pMDI_type2[action_key]['confidence_score'].append((time_val, confidence))
