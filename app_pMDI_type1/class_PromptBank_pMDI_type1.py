class PromptBank:
    def __init__(self):
        ...
        self.check_action_step_pMDI_type1 = {
            'sit_stand': {
        ...
        for q_key, action_key in q_mapping.items():
            if q_key in q_answers_accumulated:
                for answer_data in q_answers_accumulated[q_key]:
                    ...
                    self.check_action_step_pMDI_type1[action_key]['time'].append(time_val)
                    self.check_action_step_pMDI_type1[action_key]['score'].append(score)
                    
                    # confidence score 저장
                    if confidence is not None:
                        self.check_action_step_pMDI_type1[action_key]['confidence_score'].append((time_val, confidence))
    ...
    def get_fail_summary_prompt(action_decisions: dict, action_analysis: dict) -> tuple:
        ...
        action_descriptions = {
            ...
            'shake_inhaler': '흡입기 흔들기 (흡입기를 잡은 손이 연속 프레임에서 움직이는 행동)',
...
위 정보를 바탕으로 FAIL 항목별로 구체적인 문제점을 한국어로 기술 바랍니다. 
시간 정보 등을 기술하지 말고, 문제점 만을 이해하기 쉽고 간략하게 기술 바랍니다.
문제점 외의 정보는 기술하지 마세요."""
