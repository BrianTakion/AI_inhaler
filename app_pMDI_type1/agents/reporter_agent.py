...
# class_PromptBank_pMDI_type1 import
...
class ReporterAgent:
    ...
    ACTION_AGGREGATION_RULES = {
        ...
        'remove_cover': 'majority',      # 과반수가 True일 때만 True
        'inspect_mouthpiece': 'all',    # 모두 True일 때만 True
        'shake_inhaler': 'majority',    # 과반수가 True일 때만 True
    ...
    def _apply_individual_agent_rule(self, reference_times: dict, promptbank_data: dict) -> dict:
        ...
        # 데이터 가져오기
        actions_data = promptbank_data.get("check_action_step_pMDI_type1", {})
    ...
    def _evaluate_decisions(self, reference_times_avg: dict, promptbank_data_avg: dict) -> dict:
        ...
        # 데이터 가져오기
        actions_data = promptbank_data_avg.get("check_action_step_pMDI_type1", {})
    ...
    def _create_final_report(self, state: VideoAnalysisState, 
        ...
        for model_id, result in model_results.items():
            promptbank_data = result.get("promptbank_data", {})
            check_action = promptbank_data.get("check_action_step_pMDI_type1", {})
    ...
    def _create_individual_agent_visualization(self, model_id: str, model_name: str, 
        ...
        try:
            ...
            action_keys = list(check_action_step_pMDI_type1.keys())
            ...
            for key, value in search_reference_time.items():
                if value['reference_time'] >= 0:
                    ...
                    reference_times_list.append(value['reference_time'])
                    reference_y_pos.append(y_pos)
                    reference_texts.append(f"{value['reference_time']:.1f}s")
            
            # Reference time 점들
            if reference_times_list:
                fig.add_trace(go.Scatter(
                    x=reference_times_list,
                    y=reference_y_pos,
                    mode='markers+text',
            ...
            # Confidence 딕셔너리 생성
            confidence_dict = {}
            for key, value in check_action_step_pMDI_type1.items():
    ...
    def _create_visualization(self, state: VideoAnalysisState):
        ...
        try:
            ...
            check_action_step_pMDI_type1 = promptbank_data_avg["check_action_step_pMDI_type1"]
